...
const WalletModal = ({ show, handleClose }) => {
  ...
  const encryptPassword = (password) => {
    return CryptoJS.AES.encrypt(password, 'maestro_wallet_key').toString();
  };

  // Verify the password entered by the user
  const verifyPassword = (enteredPassword) => {
    const storedEncryptedPassword = localStorage.getItem('encryptedPassword');
    if (!storedEncryptedPassword) return false;
    const decryptedPassword = CryptoJS.AES.decrypt(storedEncryptedPassword, 'maestro_wallet_key').toString(CryptoJS.enc.Utf8);
...
const handleUnlockWallet = () => {
  if (verifyPassword(masterPassword)) {
    // Save the master password in localStorage or sessionStorage
    localStorage.setItem('walletSession', JSON.stringify({ masterPassword }));

    setView('account-info'); // Move to account-info after unlocking the wallet
  } else {
